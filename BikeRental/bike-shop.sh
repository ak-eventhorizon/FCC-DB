...
RENT_MENU() {
  ...
  then
    ...
    echo "$AVAILABLE_BIKES" | while read BIKE_ID BAR TYPE BAR SIZE    # Example: 14|BMX|28
    do
    ...
    # if input is not a number (не является строкой из одной или нескольких цифр)
    ...
    then
      ...
      then
        ...
        BIKE_INFO_FORMATTED=$(echo $BIKE_INFO | sed 's/ |/"/') # 28 | BMX --> 28" BMX
        # send to main menu
        MAIN_MENU "I have put you down for the $BIKE_INFO_FORMATTED Bike, $(echo $CUSTOMER_NAME | sed -r 's/^ *| *$//g')." # sed here to trim all spaces before and after customer name
      fi
    fi
  fi
}
